...
We have to find least number of intervals to finish all tasks. 
Given that, each task requires one interval, for tasks of size t, we need at least t intervals to complete it.

Now the problem is reduced to finding the minimum idle time when no task is executing.
Our result would be idle Time + totalTasks (size of task array)
...
class Solution {
   ...
   public int leastInterval(char[] tasks, int n) {
        ...
        for(int i=24; i>=0; i--){
            idleTime = idleTime - Math.min(frequency[i],maxFreq-1);
        }
